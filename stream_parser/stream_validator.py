    ...
    def validate(self, url: str, _in_fallback: bool = False) -> StreamValidationResult:
        ...
        try:
            ...
            try:
                ...
                # If we got HTML from a bare IP, it might be a Shoutcast/Icecast
                # status page — try standard stream paths.
                # Only trigger for bare IPs, never for regular domains, to avoid recursion.
                ...
                if "text/html" in lower_ct and status_code == 200:
                    ...
                    if is_bare_ip and not _in_fallback:
                        self.log(f"[VALIDATE] HTML from bare IP, trying Shoutcast/Icecast fallback: {final_url}")
